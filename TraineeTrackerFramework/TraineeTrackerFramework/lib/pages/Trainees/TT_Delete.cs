...
public class TT_Delete
{
    ...
    private string _traineesDeleteURL = AppConfigReader.TraineesDeleteURL;
    ...
    private IWebElement _confirmationHeader => _seleniumDriver.FindElement(By.CssSelector("h3"));
    ...
    public string GetURL() => $"{_traineesDeleteURL}?id={_traineeID}";
    public string GetPageTitle() => _seleniumDriver.Title;
    public string GetTitleHeader() => _titleHeader.Text;
    public string GetConfirmationText() => _confirmationHeader.Text;
    ...
    public string GetFullName(bool withTitle=false) => withTitle == true ? $"{GetTitle()} {GetFirstName()} {GetLastName()}" : $"{GetFirstName()} {GetLastName()}";
    ...
    public void ClickDelete() => _deleteButton.Click();
